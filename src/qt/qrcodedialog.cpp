...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("VLT")
    updateDisplayUnit();
}
...
{
    QString ret = QString("violetcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non VLT unit input in lnReqAmount, we generate the URI with VLT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::VLT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
