...
static int DIFFICULTYKIMOTOFORKHEIGHT = 17500;
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uVLT
...
void ThreadScriptCheck() {
    RenameThread("violetcoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1399481586, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0xac629e60 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0xac, 0x62, 0x9e, 0x60 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VioletcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VioletcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("VioletcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("violetcoin-miner");
    ...
    try { loop {
        ...
        printf("Running VioletcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VioletcoinMiner terminated\n");
        throw;
    }
}
